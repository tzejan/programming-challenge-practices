...
def longestZigZag(listofNum):
    ...
    def getKey(currSeq, potNumbers):
        return (tuple(currSeq), tuple(potNumbers))

    def isZigZag(num1, num2, num3):
        return (num2 - num1) * (num3 - num2) < 0

    def dp(currSeq, potNumbers):
        #print currSeq, potNumbers
        if getKey(currSeq, potNumbers) in memo:
        #    print currSeq, potNumbers
            return memo[getKey(currSeq, potNumbers)]
        #else:
        #    print currSeq, potNumbers

        if not potNumbers:
            memo[getKey(currSeq, potNumbers)] = currSeq
            return currSeq

        #main dp
        skipSeq = dp(currSeq[:], potNumbers[1:])
        memo[getKey(currSeq, potNumbers[1:])] = skipSeq

        conSeq = []
        if len(currSeq) < 2 or isZigZag(currSeq[-2], currSeq[-1], potNumbers[0]):
            if not currSeq or currSeq[-1] != potNumbers[0]:
                currSeq.append(potNumbers[0])
                conSeq = dp(currSeq, potNumbers[1:])
                memo[getKey(currSeq, potNumbers[1:])] = conSeq

        newSeq = dp([], potNumbers[1:])
        memo[getKey([], potNumbers[1:])] = newSeq

        maxlen = max(len(conSeq), len(newSeq), len(skipSeq))
        #if maxlen > 18:
        #    print maxlen
        if maxlen == len(conSeq):
            return conSeq
        if maxlen == len(newSeq):
            return newSeq
        if maxlen == len(skipSeq):
            return skipSeq
...
#for testcase in inputList:
    ...
    #if testcase[1] != len(result):
    #    print testcase
    #    print len(result),">>", result

for testcase in inputList:
    prevNum = 0
    for num in testcase[0]:
        print num - prevNum
        prevNum = num
