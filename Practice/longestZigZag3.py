...
def longestZigZag(listOfNum):
    ...
    def isZigZag(currSeq, number):
        if len(currSeq) < 2:
            return True
        if (currSeq[-1] - currSeq[-2]) * (number - currSeq[-1]) < 0:
            return True
    ...
    def dp(currSeq, potNumbers):
        #print currSeq, potNumbers, seq
        if getKey(currSeq, potNumbers) in memo:
            #print "memo", currSeq, potNumbers
        ...
        if (currSeq and currSeq[-1] == potNumbers[0]):
            return dp(currSeq, potNumbers[1:])

        skipSeq = dp(currSeq[:], potNumbers[1:])
        memo[getKey(currSeq[:], potNumbers[1:])] = skipSeq

        contSeq = []
        if isZigZag(currSeq, potNumbers[0]):
            currSeq.append(potNumbers[0])
            contSeq = dp(currSeq, potNumbers[1:])
            memo[getKey(currSeq, potNumbers[1:])] = contSeq

        if len(contSeq) > len(skipSeq):
            return contSeq
        else:
            return skipSeq
